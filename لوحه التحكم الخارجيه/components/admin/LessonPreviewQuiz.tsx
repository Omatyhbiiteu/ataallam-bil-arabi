...
import { X, CheckCircle, AlertCircle, Trophy, Frown, Music, Loader, RefreshCw } from 'lucide-react';
import { motion as m, AnimatePresence } from 'framer-motion';
import { useQuizLogic } from '../../hooks/useQuizLogic';
import type { Lesson } from '../../types';

export interface LessonPreviewQuizProps {
    lesson: Lesson;
    /** إغلاق طبقة الاختبار والعودة لمحتوى المعاينة */
    onClose: () => void;
    /** بعد اجتياز الاختبار: إغلاق المعاينة بالكامل */
    onExitPreview?: () => void;
}
...
export const LessonPreviewQuiz: React.FC<LessonPreviewQuizProps> = ({ lesson: activeLesson, onClose, onExitPreview }) => {
    const {
    ...
    if (qs.length === 0) {
        return (
            <div className="flex h-full min-h-[200px] flex-col items-center justify-center gap-4 p-8 text-center bg-[#0B0D17]" dir="rtl">
                <p className="text-gray-400 font-bold">لا توجد أسئلة في هذا الدرس.</p>
                <button type="button" onClick={onClose} className="px-6 py-3 rounded-xl bg-indigo-600 text-white font-bold">
                    رجوع
                </button>
            </div>
        );
    ...
    if (quizResult) {
        ...
        return (
            <div className="flex h-full min-h-0 w-full flex-col overflow-y-auto z-[60] bg-white dark:bg-[#0B0D17] items-center justify-center p-6 animate-fade-in" dir="rtl">
                <m.div
                ...
                >
                    ...
                    <div className="space-y-3 pt-2">
                        {passed ? (
                            <button
                                type="button"
                                onClick={() => (onExitPreview ? onExitPreview() : onClose())}
    ...
    if (!q) {
        return (
            <div className="flex h-full min-h-[200px] flex-col items-center justify-center gap-3 p-8 bg-[#0B0D17]" dir="rtl">
                <p className="text-red-400 font-bold text-sm">تعذر تحميل السؤال (فهرس غير صالح).</p>
                <button type="button" onClick={onClose} className="text-indigo-400 underline font-bold">رجوع</button>
            </div>
        );
    }

    const isNoteLike = q.type === 'note' || q.type === 'open';
    const canCheck =
    ...
    return (
        <div className="flex h-full min-h-0 w-full flex-col bg-[#fffdf7] dark:bg-[#0B0D17] pt-safe px-4 transition-colors duration-300 font-sans overflow-hidden">
            {/* Header */}
            <div className="h-16 flex items-center justify-between shrink-0 max-w-4xl mx-auto w-full pb-2">
                <button
                    onClick={onClose}
            ...
            {/* Scrollable Content */}
            <div className="flex-1 min-h-0 overflow-y-auto no-scrollbar py-2 pb-2" dir="rtl">
                <m.div
                    ...
                    className="max-w-3xl mx-auto min-h-min flex flex-col justify-start pt-4 pb-6"
                >
                    ...
                    <div className="w-full">
                        {q.type === 'note' ? (
                            ...
                            <div className="text-center text-gray-500 dark:text-gray-400 font-medium text-base max-w-xl mx-auto bg-white dark:bg-white/5 p-6 rounded-[var(--radius-xl)] border border-gray-100 dark:border-white/5 shadow-lg">
                                <p>سؤال مفتوح — اضغط متابعة للانتقال (يُقيَّم يدوياً للطالب).</p>
                            </div>
                        ) : (
                            <>
                                ...
                                {q.type === 'checkbox' && (
                                    <p className="text-center text-amber-600 dark:text-amber-400 font-bold text-sm px-4">
                                        نوع «عدة خيارات» — جرّبه من تطبيق الطالب. للمعاينة اضغط متابعة من الأسفل بعد التخمين.
                                    </p>
                                )}
            ...
            {/* Bottom Bar */}
            <div className="shrink-0 bg-white/90 dark:bg-[#0B0D17]/90 backdrop-blur-md border-t border-gray-200 dark:border-white/5 p-4 z-[55]">
                <div className="max-w-3xl mx-auto w-full">
                    <AnimatePresence mode="wait">
                        {isAnswered ? (
                            ...
                            >
                                {isNoteLike ? 'متابعة' : q.type === 'checkbox' ? 'تخطّي المعاينة لهذا السؤال' : 'تحقق من الإجابة'}
                            </m.button>
                        )}
                    </AnimatePresence>
                </div>
