import { Folder, Card, Story, Module } from '../types';

// --- INITIAL FOLDERS ---
export const INITIAL_FOLDERS_EN: Folder[] = [
    { id: 'system_en_1', name: 'Common Words (System)', color: 'bg-green-500', createdAt: Date.now(), isSystem: true },
    { id: 'default_en', name: 'My English Cards', color: 'bg-blue-500', createdAt: Date.now() }
];

export const INITIAL_FOLDERS_DE: Folder[] = [
    { id: 'system_de_1', name: 'Häufige Wörter (System)', color: 'bg-green-500', createdAt: Date.now(), isSystem: true },
    { id: 'default_de', name: 'Meine deutschen Karten', color: 'bg-red-500', createdAt: Date.now() }
];

// --- INITIAL CARDS ---
export const INITIAL_CARDS_EN: Card[] = [
    { id: 'en_1', folderId: 'default_en', frontText: 'مرحباً', backText: 'Hello', createdAt: Date.now(), nextReview: Date.now() - 10000, interval: 0, reviews: 0, easeFactor: 2.5, status: 'new' }
];

export const INITIAL_CARDS_DE: Card[] = [
    { id: 'de_1', folderId: 'default_de', frontText: 'مرحباً', backText: 'Hallo', createdAt: Date.now(), nextReview: Date.now() - 10000, interval: 0, reviews: 0, easeFactor: 2.5, status: 'new' }
];

export const INITIAL_STORIES_EN: Story[] = [
    {
        id: '1',
        title: 'The Lion and the Mouse',
        description: 'A classic fable about kindness.',
        level: 'Beginner',
        image: 'https://images.unsplash.com/photo-1614027164847-1b28cfe1df60?auto=format&fit=crop&q=80&w=800',
        content: `A lion was sleeping in the forest. A little mouse started playing on him. The lion woke up and was very angry. He was about to eat the mouse.

The mouse said, "Please, Mr. Lion, do not eat me. I am small, but one day I might help you." The lion laughed. "You? Help me? I am the King of the Forest!" But he let the mouse go.

Some days later, hunters caught the lion in a big net. The lion roared loudly. The mouse heard him and ran to help. The mouse chewed the ropes of the net with his sharp teeth. Soon, the lion was free.

"Thank you," said the lion. "You were right. Even a little friend can be a great friend."`,
        translation: `كان هناك أسد نائم في الغابة. بدأت فأرة صغيرة تلعب فوقه. استيقظ الأسد وكان غاضباً جداً. كان على وشك أن يأكل الفأرة.

قالت الفأرة: "أرجوك يا سيد أسد، لا تأكلني. أنا صغيرة، لكن يوماً ما قد أساعدك." ضحك الأسد. "أنتِ؟ تساعدينني؟ أنا ملك الغابة!" لكنه ترك الفأرة تذهب.

في أحد الأيام، أمسك الصيادون الأسد في شبكة كبيرة. زأر الأسد بصوت عالٍ. سمعته الفأرة وركضت للمساعدة. مضغت الفأرة حبال الشبكة بأسنانها الحادة. وسرعان ما أصبح الأسد حراً.

قال الأسد: "شكراً لك. كنتِ محقة. حتى الصديق الصغير يمكن أن يكون صديقاً عظيماً."`,
        isSystem: true,
        wordCount: 108,
        estimatedReadingTime: 2,
        difficulty: 2,
        tags: ['fable', 'animals', 'kindness'],
        viewCount: 0
    },
    {
        id: '2',
        title: 'The Lazy Boy',
        description: 'A story about a boy who learned the value of hard work.',
        level: 'Beginner',
        image: 'https://images.unsplash.com/photo-1541963463532-d68292c34b19?auto=format&fit=crop&q=80&w=800',
        content: `Jack was a very lazy boy. He liked to sleep and play video games all day. His father worked hard on a farm.

One day, his father said, "Jack, come help me in the field." Jack said, "I am too tired." His father was sad.

That night, the internet stopped working. Jack was bored. He went to the field. He saw his father looking at the stars. "Dad, why do you work so hard?" Jack asked.

His father smiled. "Because when you plant seeds, you get beautiful flowers and food. Laziness grows nothing."

Jack thought about this. The next day, he went to the field. He worked hard. It was difficult, but he felt proud. He learned that hard work brings its own happiness.`,
        translation: `كان جاك فتى كسولاً جداً. كان يحب النوم ولعب ألعاب الفيديو طوال اليوم. كان والده يعمل بجد في مزرعة.

في أحد الأيام، قال والده: "جاك، تعال ساعدني في الحقل." قال جاك: "أنا متعب جداً." حزن والده.

في تلك الليلة، توقف الإنترنت عن العمل. شعر جاك بالملل. ذهب إلى الحقل. رأى والده ينظر إلى النجوم. سأل جاك: "أبي، لماذا تعمل بجد؟"

ابتسم والده. "لأنك عندما تزرع البذور، تحصل على زهور جميلة وطعام. الكسل لا ينمي شيئاً."

فكر جاك في هذا. في اليوم التالي، ذهب إلى الحقل. عمل بجد. كان الأمر صعباً، لكنه شعر بالفخر. تعلم أن العمل الجاد يجلب سعادته الخاصة.`,
        isSystem: true,
        wordCount: 125,
        estimatedReadingTime: 2,
        difficulty: 2,
        tags: ['life-lesson', 'family', 'work'],
        viewCount: 0
    },
    {
        id: '3',
        title: 'The Time Traveler\'s Mistake',
        description: 'A scientist learns that some things should not be changed.',
        level: 'Intermediate',
        image: 'https://images.unsplash.com/photo-1451187580459-43490279c0fa?auto=format&fit=crop&q=80&w=800',
        content: `Dr. Sarah Chen had finally done it. After twenty years of research, she had built a working time machine. Her laboratory was filled with complex equipment, blinking lights, and the hum of electricity.

"I\'ll go back and prevent my father\'s accident," she thought. Her father had died in a car crash when she was ten years old. The pain had never left her.

She set the coordinates for March 15th, 1995, and pressed the activation button. The world around her blurred and twisted. When everything stopped spinning, she found herself standing on a familiar street corner.

There he was—her father, walking toward his car. "Dad!" she shouted, running toward him. "Don\'t get in that car! Please!"

Her father looked confused. "Do I know you?" he asked.

Sarah grabbed his arm. "You have to trust me. If you drive today, you\'ll have an accident."

Her father hesitated, then nodded. "Okay, I\'ll take the bus instead."

Sarah smiled with relief and returned to her time machine. But when she arrived back in the present, everything was different. Her laboratory was gone. In its place was an empty warehouse. She checked her phone—no contacts, no photos, nothing.

She realized the horrible truth: by saving her father, she had changed everything. Without the pain of losing him, she had never become a scientist. She had never built the time machine. She had erased her own achievement.

Sarah stood in the empty warehouse, tears streaming down her face. Some things, she realized, were meant to be—no matter how much they hurt.`,
        translation: `أخيراً نجحت الدكتورة سارة تشن. بعد عشرين عاماً من البحث، بنت آلة زمن تعمل. كان مختبرها مليئاً بالمعدات المعقدة والأضواء الوامضة وطنين الكهرباء.

"سأعود وأمنع حادث والدي"، فكرت. لقد مات والدها في حادث سيارة عندما كانت في العاشرة من عمرها. لم يفارقها الألم أبداً.

ضبطت الإحداثيات على 15 مارس 1995، وضغطت على زر التفعيل. تشوش العالم من حولها والتوى. عندما توقف كل شيء عن الدوران، وجدت نفسها واقفة في زاوية شارع مألوفة.

كان هناك—والدها، يمشي نحو سيارته. "أبي!" صرخت، راكضة نحوه. "لا تركب تلك السيارة! أرجوك!"

بدا والدها مرتبكاً. "هل أعرفك؟" سأل.

أمسكت سارة بذراعه. "يجب أن تثق بي. إذا قدت اليوم، ستتعرض لحادث."

تردد والدها، ثم أومأ. "حسناً، سآخذ الحافلة بدلاً من ذلك."

ابتسمت سارة بارتياح وعادت إلى آلة الزمن. لكن عندما وصلت إلى الحاضر، كان كل شيء مختلفاً. اختفى مختبرها. في مكانه كان هناك مستودع فارغ. فحصت هاتفها—لا جهات اتصال، لا صور، لا شيء.

أدركت الحقيقة المروعة: بإنقاذ والدها، غيرت كل شيء. بدون ألم فقدانه، لم تصبح عالمة أبداً. لم تبن آلة الزمن أبداً. محت إنجازها الخاص.

وقفت سارة في المستودع الفارغ، والدموع تنهمر على وجهها. بعض الأشياء، أدركت، كان من المفترض أن تكون—مهما كان الألم الذي تسببه.`,
        isSystem: true,
        wordCount: 285,
        estimatedReadingTime: 4,
        difficulty: 5,
        tags: ['science-fiction', 'time-travel', 'philosophy'],
        viewCount: 0,
        questions: [
            {
                id: 'q3_1',
                type: 'multiple-choice',
                text: 'Why did Dr. Sarah Chen build a time machine?',
                options: ['To become famous', 'To prevent her father\'s death', 'To win a Nobel Prize', 'To explore the future'],
                correctAnswer: 'To prevent her father\'s death'
            },
            {
                id: 'q3_2',
                type: 'true-false',
                text: 'Sarah successfully saved her father and kept her laboratory.',
                correctAnswer: 'false'
            },
            {
                id: 'q3_3',
                type: 'multiple-choice',
                text: 'What was the main lesson of the story?',
                options: ['Time travel is impossible', 'Some painful events shape who we become', 'Scientists should not experiment', 'The past cannot be changed'],
                correctAnswer: 'Some painful events shape who we become'
            }
        ]
    },
    {
        id: '4',
        title: 'The Last Library',
        description: 'In a digital world, one girl discovers the magic of physical books.',
        level: 'Intermediate',
        image: 'https://images.unsplash.com/photo-1521587760476-6c12a4b040da?auto=format&fit=crop&q=80&w=800',
        content: `In the year 2087, books were extinct. Everything was digital—stored in clouds, accessed through neural implants, downloaded directly into people\'s minds. Libraries were ancient history, mentioned only in digital archives.

Maya was sixteen and had never touched a physical book. She learned everything through her implant: mathematics appeared in her vision, history played like movies in her mind, languages downloaded in seconds.

One day, while exploring an abandoned part of the city, Maya discovered a hidden door. Behind it was a room filled with dusty shelves, and on those shelves were hundreds of objects she had only seen in historical files—books.

Curiously, she picked one up. It was heavy, with a worn leather cover. She opened it carefully. The pages were yellow and fragile. There were no moving images, no sound effects, no interactive elements. Just words. Black ink on paper.

Maya began to read. At first, it felt strange. Her mind was used to information appearing instantly. But as she continued, something magical happened. The words created pictures in her imagination—pictures she created herself, not downloaded from a server.

She spent hours in that hidden library, reading story after story. Each book was different. Some made her laugh, others made her cry. The experience was slower than her implant, but somehow richer, more personal.

When she finally left, Maya made a decision. She would protect this place. In a world of instant information, she had discovered something precious: the joy of discovering knowledge slowly, of creating her own mental images, of holding history in her hands.

The last library would not be forgotten.`,
        translation: `في عام 2087، انقرضت الكتب. كان كل شيء رقمياً—مخزناً في السحابة، يُصل إليه عبر غرسات عصبية، يُحمّل مباشرة في عقول الناس. كانت المكتبات تاريخاً قديماً، تُذكر فقط في الأرشيفات الرقمية.

كانت مايا في السادسة عشرة ولم تلمس كتاباً مادياً قط. تعلمت كل شيء من خلال غرستها: ظهرت الرياضيات في رؤيتها، لُعب التاريخ كأفلام في عقلها، حُملت اللغات في ثوانٍ.

ذات يوم، أثناء استكشاف جزء مهجور من المدينة، اكتشفت مايا باباً مخفياً. خلفه كانت غرفة مليئة بالرفوف المغبرة، وعلى تلك الرفوف كانت مئات الأشياء التي رأتها فقط في الملفات التاريخية—كتب.

بفضول، التقطت واحداً. كان ثقيلاً، بغلاف جلدي بالٍ. فتحته بحذر. كانت الصفحات صفراء وهشة. لم تكن هناك صور متحركة، ولا مؤثرات صوتية، ولا عناصر تفاعلية. فقط كلمات. حبر أسود على ورق.

بدأت مايا بالقراءة. في البداية، شعرت بالغرابة. كان عقلها معتاداً على ظهور المعلومات فوراً. لكن بينما واصلت، حدث شيء سحري. خلقت الكلمات صوراً في خيالها—صوراً خلقتها بنفسها، لم تُحمّل من خادم.

قضت ساعات في تلك المكتبة المخفية، تقرأ قصة تلو الأخرى. كان كل كتاب مختلفاً. بعضها أضحكها، والبعض الآخر أبكاها. كانت التجربة أبطأ من غرستها، لكنها بطريقة ما أغنى، أكثر شخصية.

عندما غادرت أخيراً، اتخذت مايا قراراً. ستحمي هذا المكان. في عالم من المعلومات الفورية، اكتشفت شيئاً ثميناً: متعة اكتشاف المعرفة ببطء، وخلق صورها الذهنية الخاصة، وحمل التاريخ بين يديها.

لن تُنسى المكتبة الأخيرة.`,
        isSystem: true,
        wordCount: 312,
        estimatedReadingTime: 5,
        difficulty: 6,
        tags: ['science-fiction', 'books', 'technology', 'future'],
        viewCount: 0,
        questions: [
            {
                id: 'q4_1',
                type: 'multiple-choice',
                text: 'How did people learn in 2087?',
                options: ['Through physical books', 'Through neural implants', 'Through teachers', 'Through television'],
                correctAnswer: 'Through neural implants'
            },
            {
                id: 'q4_2',
                type: 'text-input',
                text: 'What was special about reading physical books according to Maya?',
                correctAnswer: 'creating her own mental images'
            },
            {
                id: 'q4_3',
                type: 'true-false',
                text: 'Maya decided to destroy the library because books were outdated.',
                correctAnswer: 'false'
            }
        ]
    },
    {
        id: '5',
        title: 'The Silent Musician',
        description: 'A deaf composer creates the most beautiful symphony.',
        level: 'Intermediate',
        image: 'https://images.unsplash.com/photo-1511379938547-c1f69419868d?auto=format&fit=crop&q=80&w=800',
        content: `Elena had been deaf since birth. She lived in a world of silence, communicating through sign language and written words. Most people assumed she could never appreciate music, but they were wrong.

Elena felt music. She felt the vibrations of bass drums through the floor. She felt the rhythm of guitar strings through the wood. She watched the movements of conductors and dancers, seeing the patterns and emotions that others only heard.

At twenty-five, Elena decided to compose her own symphony. People laughed. "How can a deaf person write music?" they asked. But Elena didn\'t listen to the doubters—she couldn\'t hear them anyway.

She worked for three years, feeling every note through vibrations, watching videos of orchestras in slow motion, studying the mathematics of harmony and rhythm. She translated sound into patterns she could understand—visual patterns, numerical sequences, physical sensations.

When her symphony was finally performed, the concert hall was packed. Critics came expecting a disaster. What they heard instead was extraordinary. The music was different—it had unusual rhythms and unexpected harmonies—but it was beautiful in a way they had never experienced before.

After the performance, a famous conductor approached Elena. "How did you do it?" he asked through an interpreter.

Elena smiled and signed her response: "I didn\'t hear the music. I felt it. I saw it. I understood it in ways you never could. My silence gave me a different kind of hearing."

The conductor realized something profound: limitations can become strengths. Elena\'s deafness hadn\'t prevented her from creating music—it had given her a unique perspective that made her music unlike anything else in the world.`,
        translation: `كانت إيلينا صماء منذ الولادة. عاشت في عالم من الصمت، تتواصل من خلال لغة الإشارة والكلمات المكتوبة. افترض معظم الناس أنها لن تتمكن أبداً من تقدير الموسيقى، لكنهم كانوا مخطئين.

شعرت إيلينا بالموسيقى. شعرت باهتزازات طبول الباس عبر الأرضية. شعرت بإيقاع أوتار الجيتار عبر الخشب. شاهدت حركات القادة والراقصين، رأت الأنماط والعواطف التي سمعها الآخرون فقط.

في الخامسة والعشرين، قررت إيلينا تأليف سيمفونيتها الخاصة. ضحك الناس. "كيف يمكن لشخص أصم أن يكتب موسيقى؟" سألوا. لكن إيلينا لم تستمع للمشككين—لم تكن تستطيع سماعهم على أي حال.

عملت لمدة ثلاث سنوات، تشعر بكل نوتة من خلال الاهتزازات، تشاهد مقاطع فيديو للأوركسترا بالحركة البطيئة، تدرس رياضيات التناغم والإيقاع. ترجمت الصوت إلى أنماط يمكنها فهمها—أنماط بصرية، تسلسلات عددية، أحاسيس جسدية.

عندما عُزفت سيمفونيتها أخيراً، كانت قاعة الحفلات مكتظة. جاء النقاد متوقعين كارثة. ما سمعوه بدلاً من ذلك كان استثنائياً. كانت الموسيقى مختلفة—كان لها إيقاعات غير عادية وتناغمات غير متوقعة—لكنها كانت جميلة بطريقة لم يختبروها من قبل.

بعد الأداء، اقترب قائد أوركسترا مشهور من إيلينا. "كيف فعلتِ ذلك؟" سأل من خلال مترجم.

ابتسمت إيلينا ووقعت ردها: "لم أسمع الموسيقى. شعرت بها. رأيتها. فهمتها بطرق لا يمكنك أبداً. أعطاني صمتي نوعاً مختلفاً من السمع."

أدرك القائد شيئاً عميقاً: يمكن أن تصبح القيود نقاط قوة. لم يمنعها صمم إيلينا من خلق الموسيقى—بل أعطاها منظوراً فريداً جعل موسيقاها لا مثيل لها في العالم.`,
        isSystem: true,
        wordCount: 298,
        estimatedReadingTime: 4,
        difficulty: 6,
        tags: ['inspiration', 'music', 'disability', 'art'],
        viewCount: 0,
        questions: [
            {
                id: 'q5_1',
                type: 'multiple-choice',
                text: 'How did Elena experience music?',
                options: ['Through hearing aids', 'Through vibrations and visual patterns', 'She couldn\'t experience it', 'Through other people\'s descriptions'],
                correctAnswer: 'Through vibrations and visual patterns'
            },
            {
                id: 'q5_2',
                type: 'multiple-choice',
                text: 'What was the main message of the story?',
                options: ['Deaf people cannot create music', 'Limitations can become unique strengths', 'Music is only for those who can hear', 'Technology solves all problems'],
                correctAnswer: 'Limitations can become unique strengths'
            }
        ]
    },
    {
        id: '6',
        title: 'The Memory Thief',
        description: 'A thief who steals memories learns their true value.',
        level: 'Intermediate',
        image: 'https://images.unsplash.com/photo-1419242902214-272b3f66ee7a?auto=format&fit=crop&q=80&w=800',
        content: `In the underground markets of Neo-Tokyo, memories were currency. People sold their happiest moments to pay rent, traded childhood memories for food, exchanged love stories for medicine. And Marcus was the best memory thief in the business.

He had a special device—a neural scanner that could extract memories without the owner knowing. He targeted the wealthy, stealing their precious moments and selling them to the highest bidder. He felt no guilt. Memories were just data, he told himself.

One night, Marcus broke into the penthouse of a billionaire. The man was old, sitting alone in a dark room. Marcus activated his scanner and began the extraction. But something went wrong. Instead of stealing the memory, Marcus experienced it.

He saw the billionaire as a young man, holding his newborn daughter. The joy, the fear, the overwhelming love—Marcus felt it all. Then he saw the daughter growing up, her first steps, her graduation. He saw her wedding day, the billionaire crying tears of happiness.

Then the memory turned dark. The daughter\'s car accident. The hospital. The funeral. The billionaire\'s world shattering into pieces. Marcus felt the man\'s grief as if it were his own.

When the memory ended, Marcus was crying. He looked at the old man, who was staring at him with knowing eyes.

"You felt it, didn\'t you?" the billionaire whispered. "Now you understand. Memories aren\'t just data. They\'re pieces of our souls. The happy ones give us strength. The painful ones make us human."

Marcus left the penthouse empty-handed. He destroyed his scanner and never stole another memory. He had learned that some things have value beyond price—and memories, both joyful and painful, are what make us who we are.`,
        translation: `في أسواق نيو-طوكيو السرية، كانت الذكريات عملة. باع الناس لحظاتهم الأسعد لدفع الإيجار، تبادلوا ذكريات الطفولة بالطعام، استبدلوا قصص الحب بالدواء. وكان ماركوس أفضل سارق ذكريات في المجال.

كان لديه جهاز خاص—ماسح عصبي يمكنه استخراج الذكريات دون علم المالك. استهدف الأثرياء، سرق لحظاتهم الثمينة وباعها لأعلى مزايد. لم يشعر بالذنب. الذكريات مجرد بيانات، قال لنفسه.

ذات ليلة، اقتحم ماركوس شقة مليونير. كان الرجل عجوزاً، جالساً وحيداً في غرفة مظلمة. فعّل ماركوس ماسحه وبدأ الاستخراج. لكن حدث خطأ ما. بدلاً من سرقة الذاكرة، عاشها ماركوس.

رأى المليونير كشاب، يحمل ابنته المولودة حديثاً. الفرح، الخوف، الحب الطاغي—شعر ماركوس بكل ذلك. ثم رأى الابنة تكبر، خطواتها الأولى، تخرجها. رأى يوم زفافها، المليونير يبكي دموع السعادة.

ثم أصبحت الذاكرة مظلمة. حادث سيارة الابنة. المستشفى. الجنازة. عالم المليونير يتحطم إلى قطع. شعر ماركوس بحزن الرجل كما لو كان حزنه.

عندما انتهت الذاكرة، كان ماركوس يبكي. نظر إلى الرجل العجوز، الذي كان يحدق فيه بعيون عارفة.

"شعرت بها، أليس كذلك؟" همس المليونير. "الآن تفهم. الذكريات ليست مجرد بيانات. إنها قطع من أرواحنا. السعيدة منها تعطينا القوة. المؤلمة تجعلنا بشراً."

غادر ماركوس الشقة خالي الوفاض. دمر ماسحه ولم يسرق ذاكرة أخرى أبداً. تعلم أن بعض الأشياء لها قيمة تتجاوز السعر—والذكريات، السعيدة والمؤلمة، هي ما يجعلنا من نحن.`,
        isSystem: true,
        wordCount: 318,
        estimatedReadingTime: 5,
        difficulty: 6,
        tags: ['science-fiction', 'philosophy', 'ethics', 'emotion'],
        viewCount: 0,
        questions: [
            {
                id: 'q6_1',
                type: 'multiple-choice',
                text: 'What did Marcus do for a living?',
                options: ['He sold memories', 'He stole memories', 'He created memories', 'He deleted memories'],
                correctAnswer: 'He stole memories'
            },
            {
                id: 'q6_2',
                type: 'true-false',
                text: 'Marcus continued stealing memories after experiencing the billionaire\'s memory.',
                correctAnswer: 'false'
            },
            {
                id: 'q6_3',
                type: 'multiple-choice',
                text: 'What did Marcus learn from the experience?',
                options: ['Memories are worthless', 'Only happy memories matter', 'Memories make us human', 'Stealing is profitable'],
                correctAnswer: 'Memories make us human'
            }
        ]
    },
    {
        id: '7',
        title: 'The Paradox of Choice',
        description: 'A philosopher discovers that unlimited options can be a prison.',
        level: 'Advanced',
        image: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&q=80&w=800',
        content: `Professor David Schwartz had dedicated his life to studying decision-making. His research showed that humans, when given unlimited choices, often became paralyzed and made worse decisions than when options were limited. It was a fascinating theoretical concept—until it became his reality.

The experiment started innocuously. A tech company had developed a device that could show you every possible outcome of every decision you might make. They called it the "Omniscient Choice Engine." David volunteered to test it, believing it would revolutionize human decision-making.

The first day was exhilarating. Should he have coffee or tea? The device showed him both futures: with coffee, he'd be slightly more alert but also more anxious; with tea, he'd be calmer but less productive. He chose coffee, fascinated by the precision.

But then came bigger decisions. Which research project should he pursue? The device showed him thousands of possible futures, each branching into infinite possibilities. In one future, he won a Nobel Prize but his marriage failed. In another, he remained happily married but his career stagnated. In yet another, he achieved both but developed a chronic illness from stress.

David spent hours analyzing each option, trying to find the "perfect" choice. Days turned into weeks. He stopped making decisions altogether. He couldn't choose what to eat, what to wear, or even whether to get out of bed. Every choice led to infinite consequences, and he couldn't bear the responsibility of choosing wrong.

His wife found him one evening, staring at the device, tears streaming down his face. "I can't do it anymore," he whispered. "How do people make choices when they can see all the consequences?"

She gently took the device from his hands and turned it off. "They don't see the consequences," she said. "They make choices based on values, intuition, and hope. Perfection isn't the goal—living is."

David realized his mistake. The pursuit of the perfect choice had prevented him from making any choice at all. Freedom wasn't about having unlimited options—it was about having the courage to choose despite uncertainty.

He destroyed the device the next day. His research paper, titled "The Paradox of Perfect Information," became his most influential work. In it, he argued that human decision-making isn't flawed because we lack information—it's beautiful because we choose despite our limitations.`,
        translation: `كرّس البروفيسور ديفيد شوارتز حياته لدراسة صنع القرار. أظهرت أبحاثه أن البشر، عند منحهم خيارات غير محدودة، غالباً ما يصابون بالشلل ويتخذون قرارات أسوأ من عندما تكون الخيارات محدودة. كان مفهوماً نظرياً رائعاً—حتى أصبح واقعه.

بدأت التجربة بشكل بريء. طورت شركة تقنية جهازاً يمكنه أن يريك كل نتيجة محتملة لكل قرار قد تتخذه. أسموه "محرك الاختيار العليم". تطوع ديفيد لاختباره، معتقداً أنه سيحدث ثورة في صنع القرار البشري.

كان اليوم الأول مبهجاً. هل يجب أن يشرب قهوة أم شاياً؟ أظهر له الجهاز كلا المستقبلين: مع القهوة، سيكون أكثر يقظة قليلاً لكن أيضاً أكثر قلقاً؛ مع الشاي، سيكون أكثر هدوءاً لكن أقل إنتاجية. اختار القهوة، مفتوناً بالدقة.

لكن بعد ذلك جاءت قرارات أكبر. أي مشروع بحثي يجب أن يتابع؟ أظهر له الجهاز آلاف المستقبلات المحتملة، كل منها يتفرع إلى احتمالات لا نهائية. في مستقبل واحد، فاز بجائزة نوبل لكن زواجه فشل. في آخر، بقي متزوجاً بسعادة لكن مسيرته المهنية ركدت. في ثالث، حقق كليهما لكنه أصيب بمرض مزمن من الإجهاد.

قضى ديفيد ساعات في تحليل كل خيار، محاولاً إيجاد الاختيار "المثالي". تحولت الأيام إلى أسابيع. توقف عن اتخاذ القرارات تماماً. لم يستطع اختيار ما يأكل، أو ما يرتدي، أو حتى ما إذا كان سينهض من السرير. كل اختيار أدى إلى عواقب لا نهائية، ولم يستطع تحمل مسؤولية الاختيار الخاطئ.

وجدته زوجته ذات مساء، يحدق في الجهاز، والدموع تنهمر على وجهه. "لا أستطيع فعل ذلك بعد الآن"، همس. "كيف يتخذ الناس خيارات عندما يمكنهم رؤية جميع العواقب؟"

أخذت الجهاز من يديه بلطف وأطفأته. "إنهم لا يرون العواقب"، قالت. "يتخذون خيارات بناءً على القيم والحدس والأمل. الكمال ليس الهدف—العيش هو الهدف."

أدرك ديفيد خطأه. لقد منعه السعي وراء الاختيار المثالي من اتخاذ أي اختيار على الإطلاق. لم تكن الحرية تتعلق بامتلاك خيارات غير محدودة—بل كانت تتعلق بامتلاك الشجاعة للاختيار رغم عدم اليقين.

دمر الجهاز في اليوم التالي. أصبحت ورقته البحثية، بعنوان "مفارقة المعلومات المثالية"، أكثر أعماله تأثيراً. فيها، جادل بأن صنع القرار البشري ليس معيباً لأننا نفتقر إلى المعلومات—إنه جميل لأننا نختار رغم قيودنا.`,
        isSystem: true,
        wordCount: 425,
        estimatedReadingTime: 6,
        difficulty: 8,
        tags: ['philosophy', 'psychology', 'decision-making', 'science-fiction'],
        viewCount: 0,
        questions: [
            {
                id: 'q7_1',
                type: 'multiple-choice',
                text: 'What was Professor Schwartz\'s field of study?',
                options: ['Physics', 'Decision-making', 'Technology', 'Medicine'],
                correctAnswer: 'Decision-making'
            },
            {
                id: 'q7_2',
                type: 'multiple-choice',
                text: 'What problem did the Omniscient Choice Engine create?',
                options: ['It was too expensive', 'It showed too many possibilities and paralyzed decision-making', 'It was inaccurate', 'It was too slow'],
                correctAnswer: 'It showed too many possibilities and paralyzed decision-making'
            },
            {
                id: 'q7_3',
                type: 'text-input',
                text: 'According to the story, what makes human decision-making beautiful?',
                correctAnswer: 'choosing despite limitations'
            }
        ]
    },
    {
        id: '8',
        title: 'The Ethics of Immortality',
        description: 'A scientist must decide whether humanity should live forever.',
        level: 'Advanced',
        image: 'https://images.unsplash.com/photo-1576086213369-97a306d36557?auto=format&fit=crop&q=80&w=800',
        content: `Dr. Amara Okafor had done the impossible: she had discovered the cure for aging. Her laboratory in Lagos had become the center of the scientific world. World leaders, billionaires, and ordinary people alike begged for access to her treatment. But Amara refused to release it.

The treatment was simple—a single injection that would reset cellular aging indefinitely. No more disease, no more death from old age. Humanity could live forever. It seemed like the ultimate gift, but Amara saw the darker implications.

She convened a secret council of ethicists, economists, philosophers, and scientists. "If we release this," she began, "what happens to society?"

The economists spoke first: "Immortality for all would collapse the economy. Jobs would never open up. Young people would have no opportunities. Resources would be depleted within decades."

The philosophers countered: "But who are we to deny people the choice to live? Death is humanity's greatest tragedy. We have a moral obligation to prevent it."

A sociologist raised another concern: "What about population control? If no one dies, we'll have billions more people within a generation. The planet can't sustain that."

An ethicist added: "And what about inequality? Even if we try to distribute it fairly, the wealthy will hoard it. We'll create a permanent ruling class that never dies while the poor are left to age and die."

The debates continued for weeks. Amara listened to every argument, her burden growing heavier each day. She thought about her own grandmother, who had died peacefully at ninety-three, surrounded by family. Would that death have been better or worse if she had known she could live forever?

One night, Amara made her decision. She published her research openly, making the formula available to everyone for free. But she included a detailed analysis of the ethical implications and a recommendation: humanity should decide collectively, through democratic processes, whether and how to use it.

"I am a scientist, not a god," she wrote in her final paper. "I can give humanity the tools, but I cannot make the choice for them. The question isn't whether we can live forever—it's whether we should. And that decision belongs to all of us, not just to me."

The world erupted in debate. Some countries banned the treatment. Others made it freely available. Some created age limits—you could only take it after living a full natural life. The discussions continue to this day.

Amara herself never took the treatment. "I want to experience life as humans always have," she explained. "With a beginning, a middle, and an end. That's what makes each moment precious."`,
        translation: `فعلت الدكتورة أمارا أوكافور المستحيل: اكتشفت علاج الشيخوخة. أصبح مختبرها في لاغوس مركز العالم العلمي. توسل قادة العالم والمليارديرات والناس العاديون على حد سواء للوصول إلى علاجها. لكن أمارا رفضت إطلاقه.

كان العلاج بسيطاً—حقنة واحدة من شأنها إعادة ضبط شيخوخة الخلايا إلى أجل غير مسمى. لا مزيد من الأمراض، لا مزيد من الموت من الشيخوخة. يمكن للبشرية أن تعيش إلى الأبد. بدا وكأنه الهدية المثلى، لكن أمارا رأت الآثار الأكثر قتامة.

عقدت مجلساً سرياً من علماء الأخلاق والاقتصاديين والفلاسفة والعلماء. "إذا أطلقنا هذا"، بدأت، "ماذا يحدث للمجتمع؟"

تحدث الاقتصاديون أولاً: "الخلود للجميع سينهار الاقتصاد. لن تفتح الوظائف أبداً. لن يكون لدى الشباب فرص. ستُستنفد الموارد في غضون عقود."

رد الفلاسفة: "لكن من نحن لننكر على الناس خيار العيش؟ الموت هو أعظم مأساة للبشرية. لدينا التزام أخلاقي بمنعه."

أثار عالم اجتماع قلقاً آخر: "ماذا عن السيطرة على السكان؟ إذا لم يمت أحد، سيكون لدينا مليارات أخرى من الناس في غضون جيل. الكوكب لا يمكنه تحمل ذلك."

أضاف أخلاقي: "وماذا عن عدم المساواة؟ حتى لو حاولنا توزيعه بشكل عادل، سيكتنزه الأثرياء. سنخلق طبقة حاكمة دائمة لا تموت أبداً بينما يُترك الفقراء ليشيخوا ويموتوا."

استمرت المناقشات لأسابيع. استمعت أمارا لكل حجة، وعبئها يزداد ثقلاً كل يوم. فكرت في جدتها، التي ماتت بسلام في الثالثة والتسعين، محاطة بالعائلة. هل كان ذلك الموت سيكون أفضل أم أسوأ لو كانت تعلم أنها يمكن أن تعيش إلى الأبد؟

ذات ليلة، اتخذت أمارا قرارها. نشرت بحثها علناً، جاعلة الصيغة متاحة للجميع مجاناً. لكنها أدرجت تحليلاً مفصلاً للآثار الأخلاقية وتوصية: يجب أن تقرر البشرية بشكل جماعي، من خلال العمليات الديمقراطية، ما إذا كان وكيف تستخدمه.

"أنا عالمة، لست إلهاً"، كتبت في ورقتها النهائية. "يمكنني أن أعطي البشرية الأدوات، لكن لا يمكنني اتخاذ الاختيار نيابة عنهم. السؤال ليس ما إذا كان بإمكاننا العيش إلى الأبد—بل ما إذا كان يجب علينا. وهذا القرار يخص جميعنا، وليس لي فقط."

انفجر العالم في نقاش. حظرت بعض البلدان العلاج. جعله آخرون متاحاً بحرية. أنشأ البعض حدود عمرية—يمكنك فقط أخذه بعد عيش حياة طبيعية كاملة. تستمر المناقشات حتى يومنا هذا.

أمارا نفسها لم تأخذ العلاج أبداً. "أريد أن أختبر الحياة كما فعل البشر دائماً"، أوضحت. "مع بداية ووسط ونهاية. هذا ما يجعل كل لحظة ثمينة."`,
        isSystem: true,
        wordCount: 445,
        estimatedReadingTime: 7,
        difficulty: 9,
        tags: ['ethics', 'philosophy', 'science', 'immortality', 'society'],
        viewCount: 0,
        questions: [
            {
                id: 'q8_1',
                type: 'multiple-choice',
                text: 'What did Dr. Okafor discover?',
                options: ['A cure for cancer', 'A cure for aging', 'Time travel', 'Teleportation'],
                correctAnswer: 'A cure for aging'
            },
            {
                id: 'q8_2',
                type: 'checkbox',
                text: 'Which concerns were raised about immortality? (Select all that apply)',
                options: ['Economic collapse', 'Overpopulation', 'Inequality', 'Loss of tradition'],
                correctAnswer: ['Economic collapse', 'Overpopulation', 'Inequality']
            },
            {
                id: 'q8_3',
                type: 'multiple-choice',
                text: 'What was Dr. Okafor\'s final decision?',
                options: ['Keep the formula secret', 'Sell it to the highest bidder', 'Publish it openly and let humanity decide', 'Destroy the research'],
                correctAnswer: 'Publish it openly and let humanity decide'
            },
            {
                id: 'q8_4',
                type: 'true-false',
                text: 'Dr. Okafor took the immortality treatment herself.',
                correctAnswer: 'false'
            }
        ]
    },
    {
        id: '9',
        title: 'The Language of Silence',
        description: 'A linguist discovers that some things transcend words.',
        level: 'Advanced',
        image: 'https://images.unsplash.com/photo-1519389950473-47ba0277781c?auto=format&fit=crop&q=80&w=800',
        content: `Professor Katherine Wells had mastered forty-seven languages. She could discuss philosophy in ancient Greek, negotiate business in Mandarin, recite poetry in Arabic, and tell jokes in Swahili. Language was her life, her passion, her identity. She believed that with enough words, anything could be expressed.

Then she met the Sentinelese.

The Sentinelese people lived on a remote island, completely isolated from the modern world. They had no written language, and their spoken language was unlike anything linguists had encountered. Katherine was part of a carefully planned first contact mission, approved after decades of ethical debate.

The first meeting was tense. The Sentinelese watched the visitors with suspicion. Katherine tried every linguistic approach she knew—gestures, drawings, simple sounds. Nothing worked. The cultural and linguistic gap was too vast.

Frustrated, Katherine sat down on the beach and simply waited. An elderly Sentinelese woman approached slowly. She didn't speak. Instead, she sat beside Katherine and began drawing patterns in the sand—circles within circles, spirals, waves.

Katherine watched, mesmerized. The woman wasn't trying to communicate words. She was sharing something deeper—patterns, rhythms, the fundamental structures that existed before language. Katherine found herself drawing too, responding not with words but with shapes and movements.

They sat together for hours. No words were exchanged, yet Katherine felt she understood more than any conversation had ever taught her. The woman's drawings spoke of cycles—life and death, seasons, generations. They spoke of connection, of being part of something larger than oneself.

When the woman finally stood to leave, she placed her hand briefly on Katherine's shoulder. In that touch, Katherine felt acceptance, curiosity, and a profound sadness—perhaps an awareness that their worlds, once connected by this moment, would soon drift apart again.

Katherine returned to her university a changed person. She wrote a controversial paper titled "The Limits of Language." In it, she argued that language, for all its power, could never fully capture certain human experiences. Some things—love, grief, awe, connection—existed in the spaces between words.

Her colleagues were shocked. "But you've dedicated your life to language," they protested.

"Exactly," Katherine replied. "And that's how I know its boundaries. Language is a bridge, but it's not the only bridge. Sometimes, silence speaks louder than words. Sometimes, a touch, a look, a shared moment of existence—these communicate what dictionaries never could."

She continued teaching, but her approach changed. She taught her students not just vocabulary and grammar, but also the art of listening to what wasn't being said, of understanding the music beneath the words, of recognizing that true communication often happened in the pauses, the gestures, the unspoken acknowledgments that connected one human soul to another.`,
        translation: `أتقنت البروفيسورة كاثرين ويلز سبعاً وأربعين لغة. كان بإمكانها مناقشة الفلسفة باليونانية القديمة، والتفاوض على الأعمال بالماندرين، وتلاوة الشعر بالعربية، وإلقاء النكات بالسواحيلية. كانت اللغة حياتها، شغفها، هويتها. كانت تؤمن أنه بما يكفي من الكلمات، يمكن التعبير عن أي شيء.

ثم التقت بشعب سنتينيليز.

عاش شعب سنتينيليز في جزيرة نائية، معزولين تماماً عن العالم الحديث. لم يكن لديهم لغة مكتوبة، ولغتهم المنطوقة كانت مختلفة عن أي شيء واجهه اللغويون. كانت كاثرين جزءاً من مهمة اتصال أولى مخططة بعناية، تمت الموافقة عليها بعد عقود من النقاش الأخلاقي.

كان الاجتماع الأول متوتراً. راقب شعب سنتينيليز الزوار بشك. جربت كاثرين كل نهج لغوي تعرفه—إيماءات، رسومات، أصوات بسيطة. لم ينجح شيء. كانت الفجوة الثقافية واللغوية واسعة جداً.

محبطة، جلست كاثرين على الشاطئ وانتظرت ببساطة. اقتربت امرأة مسنة من سنتينيليز ببطء. لم تتكلم. بدلاً من ذلك، جلست بجانب كاثرين وبدأت ترسم أنماطاً في الرمال—دوائر داخل دوائر، حلزونات، أمواج.

شاهدت كاثرين، مفتونة. لم تكن المرأة تحاول التواصل بالكلمات. كانت تشارك شيئاً أعمق—أنماط، إيقاعات، الهياكل الأساسية التي كانت موجودة قبل اللغة. وجدت كاثرين نفسها ترسم أيضاً، تستجيب ليس بالكلمات بل بالأشكال والحركات.

جلسوا معاً لساعات. لم يتم تبادل كلمات، ومع ذلك شعرت كاثرين أنها فهمت أكثر مما علمتها أي محادثة على الإطلاق. تحدثت رسومات المرأة عن الدورات—الحياة والموت، الفصول، الأجيال. تحدثت عن الاتصال، عن كونك جزءاً من شيء أكبر من نفسك.

عندما وقفت المرأة أخيراً للمغادرة، وضعت يدها لفترة وجيزة على كتف كاثرين. في تلك اللمسة، شعرت كاثرين بالقبول والفضول وحزن عميق—ربما وعي بأن عالميهما، المتصلين بهذه اللحظة، سينجرفان بعيداً مرة أخرى قريباً.

عادت كاثرين إلى جامعتها شخصاً متغيراً. كتبت ورقة مثيرة للجدل بعنوان "حدود اللغة". فيها، جادلت بأن اللغة، على الرغم من قوتها، لا يمكنها أبداً التقاط بعض التجارب البشرية بالكامل. بعض الأشياء—الحب، الحزن، الرهبة، الاتصال—كانت موجودة في المساحات بين الكلمات.

صُدم زملاؤها. "لكنك كرست حياتك للغة"، احتجوا.

"بالضبط"، أجابت كاثرين. "ولهذا أعرف حدودها. اللغة جسر، لكنها ليست الجسر الوحيد. أحياناً، يتحدث الصمت بصوت أعلى من الكلمات. أحياناً، لمسة، نظرة، لحظة مشتركة من الوجود—هذه تتواصل بما لا يمكن للقواميس أبداً."

واصلت التدريس، لكن نهجها تغير. علمت طلابها ليس فقط المفردات والقواعد، ولكن أيضاً فن الاستماع لما لا يُقال، وفهم الموسيقى تحت الكلمات، والاعتراف بأن التواصل الحقيقي غالباً ما يحدث في الوقفات، والإيماءات، والاعترافات غير المنطوقة التي تربط روحاً بشرية بأخرى.`,
        isSystem: true,
        wordCount: 468,
        estimatedReadingTime: 7,
        difficulty: 9,
        tags: ['linguistics', 'philosophy', 'communication', 'culture', 'anthropology'],
        viewCount: 0,
        questions: [
            {
                id: 'q9_1',
                type: 'multiple-choice',
                text: 'How many languages had Professor Wells mastered?',
                options: ['Twenty-seven', 'Thirty-seven', 'Forty-seven', 'Fifty-seven'],
                correctAnswer: 'Forty-seven'
            },
            {
                id: 'q9_2',
                type: 'multiple-choice',
                text: 'How did the Sentinelese woman communicate with Katherine?',
                options: ['Through spoken words', 'Through drawings in the sand', 'Through sign language', 'Through writing'],
                correctAnswer: 'Through drawings in the sand'
            },
            {
                id: 'q9_3',
                type: 'text-input',
                text: 'What was the title of Katherine\'s controversial paper?',
                correctAnswer: 'The Limits of Language'
            },
            {
                id: 'q9_4',
                type: 'multiple-choice',
                text: 'What did Katherine learn from her experience?',
                options: ['Language is useless', 'Some experiences transcend words', 'All cultures are the same', 'Silence is always better than words'],
                correctAnswer: 'Some experiences transcend words'
            }
        ]
    },
    {
        id: '10',
        title: 'The Weight of Knowledge',
        description: 'A historian discovers that knowing the past can be a burden.',
        level: 'Advanced',
        image: 'https://images.unsplash.com/photo-1457369804613-52c61a468e7d?auto=format&fit=crop&q=80&w=800',
        content: `Dr. James Morrison was the world's leading expert on World War II. He had read every document, interviewed every surviving veteran, visited every battlefield. He knew the war in intimate, horrifying detail—not just the grand strategies and famous battles, but the individual stories of suffering, courage, and moral compromise.

At first, this knowledge felt like a sacred trust. He was preserving memory, ensuring that the lessons of history would never be forgotten. But as the years passed, the weight of that knowledge began to crush him.

He knew about the young German soldier who had written letters home about his love of poetry, who had died in the mud of Stalingrad, believing he was defending his homeland. He knew about the American pilot who had dropped bombs on civilian targets and never forgave himself. He knew about the Japanese family who had starved in the final months of the war, their last diary entries describing their love for each other even as they died.

These weren't statistics. They were people—real people with hopes, fears, and families. And James carried all their stories inside him.

One day, a student asked him a simple question: "Professor, if you could go back and prevent World War II, would you?"

James paused. The obvious answer was yes. But he knew history too well. "If I prevented World War II," he said slowly, "I would erase millions of lives. Not just the deaths—the births too. Every person born after 1945 exists because of the specific sequence of events that followed the war. Change that, and you change everything."

The student looked confused. "But wouldn't that be worth it to save all those who died?"

"Would it?" James asked. "Who am I to decide that the people who exist now are less valuable than the people who might have existed in an alternate timeline? Every life is precious—the ones lost in the war, and the ones born because of it."

That night, James had a breakdown. The weight of knowing too much, of understanding the impossible complexity of cause and effect, of recognizing that history wasn't a simple story of good versus evil but an intricate web of human choices and consequences—it was too much.

His therapist helped him understand something crucial: knowledge without wisdom is a burden. It wasn't enough to know what happened. He needed to understand why it mattered, how to use that knowledge to build a better future, and when to accept that some questions have no perfect answers.

James changed his teaching approach. He still taught the facts, but he also taught humility—the recognition that history is complex, that moral judgments are difficult, and that the past should inform the future without paralyzing it.

"We study history," he told his students in his final lecture before retirement, "not to find simple answers, but to understand complex questions. We remember the past not to live in it, but to ensure we make better choices going forward. And we carry the weight of that knowledge not as a burden, but as a responsibility—to honor those who came before by creating a world they would be proud of."`,
        translation: `كان الدكتور جيمس موريسون الخبير الرائد في العالم في الحرب العالمية الثانية. قرأ كل وثيقة، أجرى مقابلات مع كل محارب قديم على قيد الحياة، زار كل ساحة معركة. عرف الحرب بتفاصيل حميمة ومرعبة—ليس فقط الاستراتيجيات الكبرى والمعارك الشهيرة، ولكن القصص الفردية للمعاناة والشجاعة والتسوية الأخلاقية.

في البداية، شعر بأن هذه المعرفة أمانة مقدسة. كان يحفظ الذاكرة، يضمن أن دروس التاريخ لن تُنسى أبداً. لكن مع مرور السنين، بدأ ثقل تلك المعرفة يسحقه.

عرف عن الجندي الألماني الشاب الذي كتب رسائل إلى المنزل عن حبه للشعر، الذي مات في طين ستالينغراد، معتقداً أنه يدافع عن وطنه. عرف عن الطيار الأمريكي الذي ألقى قنابل على أهداف مدنية ولم يسامح نفسه أبداً. عرف عن العائلة اليابانية التي جاعت في الأشهر الأخيرة من الحرب، مدخلات يومياتهم الأخيرة تصف حبهم لبعضهم البعض حتى وهم يموتون.

لم تكن هذه إحصائيات. كانوا أشخاصاً—أشخاص حقيقيين بآمال ومخاوف وعائلات. وحمل جيمس كل قصصهم بداخله.

ذات يوم، سأله طالب سؤالاً بسيطاً: "بروفيسور، إذا كان بإمكانك العودة ومنع الحرب العالمية الثانية، هل ستفعل؟"

توقف جيمس. كانت الإجابة الواضحة نعم. لكنه عرف التاريخ جيداً جداً. "إذا منعت الحرب العالمية الثانية"، قال ببطء، "سأمحو ملايين الأرواح. ليس فقط الوفيات—الولادات أيضاً. كل شخص وُلد بعد 1945 موجود بسبب التسلسل المحدد للأحداث التي تلت الحرب. غيّر ذلك، وستغير كل شيء."

بدا الطالب مرتبكاً. "لكن ألن يكون ذلك يستحق إنقاذ كل أولئك الذين ماتوا؟"

"هل سيكون؟" سأل جيمس. "من أنا لأقرر أن الأشخاص الموجودين الآن أقل قيمة من الأشخاص الذين كان يمكن أن يوجدوا في جدول زمني بديل؟ كل حياة ثمينة—تلك المفقودة في الحرب، وتلك المولودة بسببها."

في تلك الليلة، أصيب جيمس بانهيار. ثقل معرفة الكثير، وفهم التعقيد المستحيل للسبب والنتيجة، والاعتراف بأن التاريخ لم يكن قصة بسيطة عن الخير مقابل الشر بل شبكة معقدة من الخيارات والعواقب البشرية—كان كثيراً جداً.

ساعده معالجه على فهم شيء حاسم: المعرفة بدون حكمة عبء. لم يكن كافياً معرفة ما حدث. كان بحاجة إلى فهم لماذا يهم، وكيفية استخدام تلك المعرفة لبناء مستقبل أفضل، ومتى يقبل أن بعض الأسئلة ليس لها إجابات مثالية.

غيّر جيمس نهج تدريسه. ما زال يعلم الحقائق، لكنه علم أيضاً التواضع—الاعتراف بأن التاريخ معقد، وأن الأحكام الأخلاقية صعبة، وأن الماضي يجب أن يُعلم المستقبل دون شله.

"ندرس التاريخ"، قال لطلابه في محاضرته الأخيرة قبل التقاعد، "ليس للعثور على إجابات بسيطة، بل لفهم أسئلة معقدة. نتذكر الماضي ليس للعيش فيه، بل لضمان اتخاذ خيارات أفضل للمضي قدماً. ونحمل ثقل تلك المعرفة ليس كعبء، بل كمسؤولية—لتكريم أولئك الذين سبقونا بخلق عالم سيفخرون به."`,
        isSystem: true,
        wordCount: 492,
        estimatedReadingTime: 7,
        difficulty: 9,
        tags: ['history', 'philosophy', 'ethics', 'war', 'responsibility'],
        viewCount: 0,
        questions: [
            {
                id: 'q10_1',
                type: 'multiple-choice',
                text: 'What was Dr. Morrison\'s area of expertise?',
                options: ['Ancient Rome', 'World War I', 'World War II', 'The Cold War'],
                correctAnswer: 'World War II'
            },
            {
                id: 'q10_2',
                type: 'multiple-choice',
                text: 'Why couldn\'t Dr. Morrison simply say he would prevent WWII?',
                options: ['He supported the war', 'Changing history would erase people born after 1945', 'It was impossible', 'He didn\'t care'],
                correctAnswer: 'Changing history would erase people born after 1945'
            },
            {
                id: 'q10_3',
                type: 'text-input',
                text: 'According to the story, what is knowledge without wisdom?',
                correctAnswer: 'a burden'
            },
            {
                id: 'q10_4',
                type: 'multiple-choice',
                text: 'What did Dr. Morrison teach his students in his final lecture?',
                options: ['History is simple', 'We should forget the past', 'History helps us make better future choices', 'The past doesn\'t matter'],
                correctAnswer: 'History helps us make better future choices'
            }
        ]
    }
];

export const INITIAL_STORIES_DE: Story[] = [
    {
        id: 'de_1',
        title: 'Rotkäppchen (Little Red Riding Hood)',
        description: 'Ein klassisches Märchen über ein kleines Mädchen.',
        level: 'Beginner',
        image: 'https://images.unsplash.com/photo-1606210122158-eeb10e0823bf?auto=format&fit=crop&q=80&w=800',
        content: `Es war einmal ein kleines Mädchen. Alle nannten es Rotkäppchen, weil es immer eine rote Kappe trug.

Eines Tages sagte die Mutter: "Rotkäppchen, hier ist ein Korb mit Kuchen und Wein. Bring ihn der Großmutter. Sie ist krank."

Rotkäppchen ging in den Wald. Dort traf sie den Wolf. Der Wolf war sehr höflich, aber er war böse.

"Guten Tag, Rotkäppchen", sagte der Wolf. "Wohin gehst du?"
"Zur Großmutter", antwortete Rotkäppchen.

Der Wolf rannte schnell zum Haus der Großmutter. Er fraß die Großmutter und legte sich in ihr Bett. Als Rotkäppchen kam, fragته: "Großmutter, warum hast du so große Ohren?"
"Damit ich dich besser hören kann!", sagte der Wolf.

Aber zum Glück kam ein Jäger vorbei. Er rettete Rotkäppchen und die Großmutter. Und sie lebten glücklich bis ans Ende ihrer Tage.`,
        translation: `كان يا ما كان هناك فتاة صغيرة. كان الجميع يسمونها ذات الرداء الأحمر، لأنها كانت ترتدي دائماً قبعة حمراء.

في أحد الأيام قالت الأم: "ذات الرداء الأحمر، هذا سلة بها كعك ونبيذ. خذيها للجدة. إنها مريضة."

ذهبت ذات الرداء الأحمر إلى الغابة. هناك التقت بالذئب. كان الذئب مهذباً جداً، لكنه كان شريراً.

قال الذئب: "طاب يومك يا ذات الرداء الأحمر. إلى أين تذهبين؟"
أجابت: "إلى الجدة".

ركض الذئب بسرعة إلى منزل الجدة. أكل الجدة واستلقى في سريرها. عندما وصلت ذات الرداء الأحمر، سألت: "جدتي، لماذا أذناك كبيرتان؟"
قال الذئب: "لكي أسمعك بشكل أفضل!"

لكن لحسن الحظ مر صياد. أنقذ ذات الرداء الأحمر والجدة. وعاشوا في سعادة دائمة.`,
        isSystem: true
    },
    {
        id: 'de_2',
        title: 'Der faule Hans (Lazy Hans)',
        description: 'Eine Geschichte über Arbeit.',
        level: 'Beginner',
        image: 'https://images.unsplash.com/photo-1541963463532-d68292c34b19?auto=format&fit=crop&q=80&w=800',
        content: `Hans war ein sehr fauler Junge. Er schlief gerne lange. Sein Vater arbeitete hart.

Eines Tages sagte der Vater: "Hans, hilf mir bitte." Hans sagte: "Ich bin zu müde."

Aber dann lernte Hans eine Lektion. Arbeit macht stark und glücklich. Er begann zu arbeiten und fühlte sich gut.

Ende gut, alles gut.`,
        translation: `كان هانز فتى كسولاً جداً. كان يحب النوم طويلاً. كان والده يعمل بجد.

في أحد الأيام قال الأب: "هانز، ساعدني من فضلك." قال هانز: "أنا متعب جداً."

ولكن بعد ذلك تعلم هانز درساً. العمل يجعلك قوياً وسعيداً. بدأ العمل وشعر بالرضا.

النهاية السعيدة.`,
        isSystem: true
    }
];

export const INITIAL_CURRICULUM_EN: Module[] = [
    {
        id: 'mod-en-1',
        title: 'English Basics (أساسيات الإنجليزية)',
        level: 'A1',
        subLevel: 'A1.1',
        lessons: [
            { id: 'l-en-1', title: 'The Alphabet', description: 'Learn the ABCs.', duration: '10 min', content: 'A, B, C, D...', image: 'https://images.unsplash.com/photo-1503676260728-1c00da094a0b?auto=format&fit=crop&q=80&w=800' },
            { id: 'l-en-2', title: 'Greetings', description: 'How to say Hello.', duration: '15 min', content: 'Hello, Good Morning...', image: 'https://images.unsplash.com/photo-1521791136064-7986c2920216?auto=format&fit=crop&q=80&w=800' },
        ]
    },
    {
        id: 'mod-en-2',
        title: 'Basic Conversations (محادثات بسيطة)',
        level: 'A1',
        subLevel: 'A1.2',
        lessons: [
            { id: 'l-en-3', title: 'Introductions', description: 'Introduce yourself.', duration: '12 min', content: 'My name is...', image: 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?auto=format&fit=crop&q=80&w=800' },
        ]
    },
    {
        id: 'mod-en-3',
        title: 'Daily Routine (الروتين اليومي)',
        level: 'A2',
        subLevel: 'A2.1',
        lessons: [
            { id: 'l-en-4', title: 'Simple Present', description: 'Talking about habits.', duration: '20 min', content: 'I wake up at...', image: 'https://images.unsplash.com/photo-1506784983877-45594efa4cbe?auto=format&fit=crop&q=80&w=800' },
        ]
    }
];

export const INITIAL_CURRICULUM_DE: Module[] = [
    {
        id: 'mod-de-1',
        title: 'Grundlagen (الأساسيات الألمانية)',
        level: 'A1',
        subLevel: 'A1.1',
        lessons: [
            { id: 'l-de-1', title: 'Das Alphabet', description: 'Lerne das deutsche Alphabet.', duration: '10 min', content: 'A, B, C, D, E...', image: 'https://images.unsplash.com/photo-1503676260728-1c00da094a0b?auto=format&fit=crop&q=80&w=800' },
            { id: 'l-de-2', title: 'Begrüßungen', description: 'Wie sagt man Hallo?', duration: '15 min', content: 'Hallo, Guten Morgen, Guten Tag...', image: 'https://images.unsplash.com/photo-1521791136064-7986c2920216?auto=format&fit=crop&q=80&w=800' },
        ]
    },
    {
        id: 'mod-de-2',
        title: 'Erste Gespräche (محادثات أولية)',
        level: 'A1',
        subLevel: 'A1.2',
        lessons: [
            { id: 'l-de-3', title: 'Vorstellung', description: 'Sich vorstellen.', duration: '12 min', content: 'Ich heiße...', image: 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?auto=format&fit=crop&q=80&w=800' },
        ]
    },
    {
        id: 'mod-de-3',
        title: 'Alltag (الحياة اليومية)',
        level: 'A2',
        subLevel: 'A2.1',
        lessons: [
            { id: 'l-de-4', title: 'Tagesablauf', description: 'Über den Tag sprechen.', duration: '20 min', content: 'Ich stehe auf...', image: 'https://images.unsplash.com/photo-1506784983877-45594efa4cbe?auto=format&fit=crop&q=80&w=800' },
        ]
    }
];

export const INITIAL_TICKETS: any[] = [
    {
        id: 't-1',
        userId: 'user-1',
        userName: 'أحمد محمد',
        subject: 'مشكلة في تشغيل الصوت',
        status: 'open',
        priority: 'high',
        createdAt: new Date(Date.now() - 1000 * 60 * 60 * 2).toISOString(), // 2 hours ago
        lastUpdate: new Date(Date.now() - 1000 * 60 * 60 * 2).toISOString(),
        messages: [
            { id: 'm-1', sender: 'user', text: 'مرحباً، حاولت تشغيل الصوت في القصة الثالثة ولكن لا يعمل. جربت متصفح كروم وفايرفوكس.', timestamp: new Date(Date.now() - 1000 * 60 * 60 * 2).toISOString() }
        ]
    },
    {
        id: 't-2',
        userId: 'user-2',
        userName: 'سارة علي',
        subject: 'استفسار عن الاشتراكات',
        status: 'in_progress',
        priority: 'medium',
        createdAt: new Date(Date.now() - 1000 * 60 * 60 * 24).toISOString(), // 1 day ago
        lastUpdate: new Date(Date.now() - 1000 * 60 * 60 * 5).toISOString(),
        messages: [
            { id: 'm-2', sender: 'user', text: 'هل يمكنني الدفع عن طريق فودافون كاش؟', timestamp: new Date(Date.now() - 1000 * 60 * 60 * 24).toISOString() },
            { id: 'm-3', sender: 'admin', text: 'أهلاً سارة، نعم متاح. يمكنك التحويل على الرقم الموجود في صفحة الإعدادات.', timestamp: new Date(Date.now() - 1000 * 60 * 60 * 5).toISOString() }
        ]
    },
    {
        id: 't-3',
        userId: 'user-3',
        userName: 'خالد عمر',
        subject: 'اقتراح لإضافة قصص جديدة',
        status: 'resolved',
        priority: 'low',
        createdAt: new Date(Date.now() - 1000 * 60 * 60 * 48).toISOString(), // 2 days ago
        lastUpdate: new Date(Date.now() - 1000 * 60 * 60 * 40).toISOString(),
        messages: [
            { id: 'm-4', sender: 'user', text: 'ياريت تضيفوا قصص خيال علمي أكثر.', timestamp: new Date(Date.now() - 1000 * 60 * 60 * 48).toISOString() },
            { id: 'm-5', sender: 'admin', text: 'شكراً لاقتراحك يا خالد! سنقوم بإضافة مجموعة جديدة قريباً.', timestamp: new Date(Date.now() - 1000 * 60 * 60 * 40).toISOString() }
        ]
    }
];

/** افتراضي فارغ — المحتوى من الخادم عبر لوحة المسئول */
export const INITIAL_SENTENCE_TOPICS: any[] = [];

