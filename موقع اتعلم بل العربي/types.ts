...
/** مهام يومية (مجتمع) — مكافأة XP مرة واحدة يومياً لكل مهمة، منفصلة لكل مستخدم ولغة في التخزين */
export interface DailyMissionState {
  bonusXp: number;
  date: string;
  claimedReview: boolean;
  claimedStory: boolean;
  claimedMastery: boolean;
  storiesToday: number;
  masteriesToday: number;
}

export const INITIAL_DAILY_MISSION: DailyMissionState = {
  bonusXp: 0,
  date: '',
  claimedReview: false,
  claimedStory: false,
  claimedMastery: false,
  storiesToday: 0,
  masteriesToday: 0,
};
...
export interface Folder {
  ...
  /** إن وُجد وطابق المستخدم الحالي فالمجلد ملك المستخدم (من الخادم) */
  userId?: string | null;
}

/** نتيجة إضافة بطاقة: `pro_limit` = رفض بسبب حد الخطة المجانية ويُفضَّل إغلاق المودال وإظهار ترقية Pro */
export type AddCardResult = boolean | 'pro_limit';
...
export interface Card {
  ...
  /** مالك البطاقة على الخادم (للمحتوى الذي أنشأه المستخدم) */
  userId?: string | null;
...
export interface AppNotification {
  ...
  /** مرتبط بتذكرة دعم عندما يأتي الإشعار من الخادم */
  ticketId?: string | null;
  /** مرتبط بإشعار بث (Broadcast) عندما يأتي الإشعار من الخادم */
...
export interface CardImageQuota {
  allowed: boolean;
  plan: 'free' | 'silver' | 'pro' | 'enterprise';
  used: number;
  limit: number | null;
...
export interface SentenceTopic {
  id: string;
  /** لغة الجمل والوسائط (إنجليزي / ألماني) — يطابق لغة التعلّم عند العرض */
