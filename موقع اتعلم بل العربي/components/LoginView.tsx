...
export const LoginView: React.FC<LoginViewProps> = ({ onLoginSuccess, onBackToHome, onNavigateToSignup, onForgotPassword, isDarkMode, toggleTheme, langAvailability }) => {
    ...
    const handleFacebookLogin = async () => {
        ...
        try {
            ...
            setError(err?.message || 'فشل الاتصال بـ Facebook');
        } finally {
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center p-4 md:p-12 relative min-h-[100dvh] lg:min-h-0">
                ...
                <div className="w-full max-w-md animate-fade-in mt-16 md:mt-0 mb-8 md:mb-0">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
                        ...
                        <div className="space-y-1.5 md:space-y-2">
                            <div className="flex justify-between items-center">
                                ...
                                <button type="button" onClick={onForgotPassword} className="text-[10px] md:text-xs font-bold text-primary hover:underline focus:outline-none">نسيت كلمة المرور؟</button>
                            </div>
                            <div className="relative">
                    ...
                    <div className="relative my-6 md:my-8">
                        ...
                        <div className="relative flex justify-center text-xs md:text-sm"><span className="px-4 bg-white dark:bg-gray-900 text-gray-500 font-bold">أو تابع باستخدام</span></div>
                    </div>

                    <div className="grid grid-cols-2 gap-3 md:gap-4">
                        <button onClick={handleGoogleLogin} disabled={googleLoading} className="flex items-center justify-center gap-2 py-2.5 md:py-3 border border-gray-200 dark:border-gray-700 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition font-bold text-gray-700 dark:text-white text-sm md:text-base">
                            {googleLoading ? <Loader className="animate-spin w-4 h-4" /> : (
                                <>
                        ...
                        <button onClick={handleFacebookLogin} disabled={facebookLoading} className="flex items-center justify-center gap-2 py-2.5 md:py-3 border border-gray-200 dark:border-gray-700 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition font-bold text-gray-700 dark:text-white text-sm md:text-base">
                            {facebookLoading ? <Loader className="animate-spin w-4 h-4" /> : (
                                <>
                                    <svg viewBox="0 0 48 48" width="20px" height="20px">
